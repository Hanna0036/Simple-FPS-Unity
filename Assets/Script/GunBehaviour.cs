...
public class GunBehaviour : MonoBehaviour
{
    ...
    void Fire()
    {
        ...
        if(Physics.Raycast(ray, out hit, this.range))
        {
            ...
            {
                // Cek apakah punya BallTarget
                BallTarget target = hit.collider.GetComponent<BallTarget>();
                if(target != null)
                {
                    target.GetHit(); // Respawn ke posisi random
                }
                else
                {
                    Destroy(hit.collider.gameObject); // Objek lain tetap dihapus
                }
            }
        }
        else
